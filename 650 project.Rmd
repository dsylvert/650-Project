---
title: "650 Project"
author: "Jess"
date: "11/28/2021"
output: html_document
---

```{r}
#load packages
#install.packages('ggplot2','dplyr','tidyverse','readxl','patchwork','lme4','effects',"devtools","emmeans","gridExtra") #install packages

library.list<- c('ggplot2','readxl','dplyr','tidyverse','patchwork','lme4','effects','sjPlot',"emmeans","gridExtra") #load packages
sapply(library.list, library, character.only=T, quietly=T)
library(NHANES)
dat<-NHANES
```

```{r}
quantile(dat$BMI, na.rm = TRUE)
dat$bmi_quan<-ifelse(dat$BMI<21.58, 1,
                            ifelse(21.58<=dat$BMI & dat$BMI<25.98,2,
                                   ifelse(25.98<=dat$BMI & dat$BMI<30.89, 3,
                                   ifelse(dat$BMI>=30.89,4,NA))))
dat2 <- dat %>% select(ID,BPSysAve,BMI,bmi_quan,Age,Gender,Race1,Education,DirectChol,Diabetes,PhysActiveDays,SmokeNow,TotChol,AlcoholYear)

#target_pop: nonmissing outcome and POI
target_pop<-dat2[!is.na(dat2$BPSysAve)&!is.na(dat2$BMI),]#8487
#Complete cases for all variables
comp_cases<-target_pop[complete.cases(target_pop),]#1276
```

## First lets look at a Table for the dataset we will be using (complete cases: comp_cases)
```{r}
#install.packages("qwraps2")
library("qwraps2")
orig_opt <- options()$qwraps2_markup
options(qwraps2_markup = "markdown")
summarytab<-
  list("Average Systolic Blood Pressure" =
       list("mean (sd)" = ~ qwraps2::mean_sd(BPSysAve,na_rm=TRUE)),
       "BMI" =
       list("mean (sd)" = ~ qwraps2::mean_sd(BMI,na_rm=TRUE)),
       "Age" =
       list("min"       = ~ min(Age),
            "max"       = ~ max(Age),
            "mean (sd)" = ~ qwraps2::mean_sd(Age,na_rm=TRUE)),
       "Gender" =
       list("Male" = ~ qwraps2::n_perc0(Gender == "male",na_rm=TRUE),
            "Female"  = ~ qwraps2::n_perc0(Gender == "female",na_rm=TRUE)),
       "Race" =
       list("White" = ~ qwraps2::n_perc0(Race1 == "White",na_rm=TRUE),
            "Black"  = ~ qwraps2::n_perc0(Race1 == "Black",na_rm=TRUE),
            "Mexican"= ~ qwraps2::n_perc0(Race1 == "Mexican",na_rm=TRUE),
            "Hispanic"= ~ qwraps2::n_perc0(Race1 == "Hispanic",na_rm=TRUE),
            "Other" = ~ qwraps2::n_perc0(Race1 == "Other",na_rm=TRUE)),
        "Eduaction" =
       list("8th Grade" = ~ qwraps2::n_perc0(Education == "8th Grade",na_rm=TRUE),
            "9 - 11th Grade"  = ~ qwraps2::n_perc0(Education == "9 - 11th Grade",na_rm=TRUE),
            "High School"= ~ qwraps2::n_perc0(Education == "High School",na_rm=TRUE),
            "Some College"= ~ qwraps2::n_perc0(Education == "Some College",na_rm=TRUE),
            "College Grad" = ~ qwraps2::n_perc0(Education == "College Grad",na_rm=TRUE),
            "Missing" = ~ qwraps2::n_perc0(is.na(Education))),
       "HDL Cholesterol" =
       list("mean (sd)" = ~ qwraps2::mean_sd(DirectChol,na_rm=TRUE)),
       "Diabetes" =
       list("Has diabetes" = ~ qwraps2::n_perc0(Diabetes == "Yes",na_rm=TRUE),
            "Doesn't have diabetes"  = ~ qwraps2::n_perc0(Diabetes == "No",na_rm=TRUE),
            "Missing" = ~ qwraps2::n_perc0(is.na(Diabetes))),
       "Physical Activity " =
       list("mean (sd)" = ~ qwraps2::mean_sd(PhysActiveDays,na_rm=TRUE)),
        "Smoking" =
       list("Smoker" = ~ qwraps2::n_perc0(SmokeNow == "Yes",na_rm=TRUE),
            "Nonsmoker"  = ~ qwraps2::n_perc0(SmokeNow == "No",na_rm=TRUE),
            "Missing" = ~ qwraps2::n_perc0(is.na(SmokeNow))),
        "Alcohol use per year"=
            list("mean (sd)" = ~ qwraps2::mean_sd(AlcoholYear,na_rm=TRUE))
       )
#only for those with Systolic Blood Pressure 
overall<-summary_table(comp_cases,summarytab)
bmi_quan<-summary_table(comp_cases,summarytab, by=c("bmi_quan"))
comp_tab<-cbind(overall,bmi_quan)
print(comp_tab,
      title = "Summary Statistics For Complete Cases",
      cnames = c("Overall", "BMI Quartile 1", "BMI Quartile 2", "BMI Quartile 3","BMI Quartile 4"))
```
## Then lets look at a Table for the dataset of the target population (anyone in NHANES with our outcome and predictor of interest) (total cases: target_pop)
```{r}
#for target population: look at overall
overall_tar<-summary_table(target_pop,summarytab)
gen_tab<-rbind(table(target_pop$Gender),table(comp_cases$Gender))
race_tab<-rbind(table(target_pop$Race1),table(comp_cases$Race1))
Edu_tab<-rbind(table(target_pop$Education),table(comp_cases$Education))
dia_tab<-rbind(table(target_pop$Diabetes),table(comp_cases$Diabetes))
smoke_tab<-rbind(table(target_pop$SmokeNow),table(comp_cases$SmokeNow))

# bmi_quan_tar<-summary_table(target_pop,summarytab, by=c("bmi_quan"))
pval <-round(c(t.test(target_pop$BPSysAve,comp_cases$BPSysAve)$p.value, 
          t.test(target_pop$BMI,comp_cases$BMI)$p.value, NA, NA,
              t.test(target_pop$Age,comp_cases$Age)$p.value, 
              chisq.test(gen_tab)$p.value,NA,
              chisq.test(race_tab)$p.value,NA,NA,NA,NA,
              chisq.test(Edu_tab)$p.value,NA,NA,NA,NA,NA,
              t.test(target_pop$DirectChol,comp_cases$DirectChol)$p.value,
              chisq.test(dia_tab)$p.value,NA,NA,
              t.test(target_pop$PhysActiveDays,comp_cases$PhysActiveDays)$p.value,
              chisq.test(smoke_tab)$p.value, NA,NA,
              t.test(target_pop$AlcoholYear,comp_cases$AlcoholYear)$p.value),digits=3)
pval<-ifelse(is.na(pval), " ",ifelse(pval<0.001,"<0.001",pval))

Tab_target<-cbind(overall_tar,overall, "P-Value" = "")# create empty column
Tab_target[55:81]<-pval
var_nm<-c("Average Systolic Blood Pressure",
       "BMI",
       "Age",
       "Gender",
       "Race",
        "Eduaction",
       "HDL Cholesterol",
       "Diabetes",
       "Physical Activity ",
        "Smoking",
        "Alcohol use per year")
pvalues<-cbind(var_nm,pval)
pvalues
Tab_target
print(Tab_target,
      rtitle = "Summary Statistics",
      cnames = c("Total Cases(N=8487)","Complete Cases(N=1276)","P Value"))

```
## Lets look at SBP and BMI
```{r}
comp_cases%>%ggplot(aes(x=BMI,
                        y=BPSysAve))+
              geom_point()
```

## I then fit an unadjusted model
```{r}
##Just Outcome and POI
model_1<-lm(BPSysAve~BMI,data=comp_cases)
summary(model_1)
```
BMI is significant: there is an association between SBP and BMI

##I added the demographic variables next
```{r}
#I added the other covariates in groups, in a sequential pattern
#Demographic
#comp_cases <- within(comp_cases, Race1 <- relevel(Race1, ref = "White"))
model_2<-lm(BPSysAve~BMI+Gender+Race1+Education+Age,data=comp_cases)
summary(model_2)
```
BMI is still significant.

##Lets include health confounders as well
```{r}
#health variables:DirectChol+Diabetes+PhysActiveDays+SmokeNow
model_3<-lm(BPSysAve~BMI+Age+Gender+Race1+Education+DirectChol+Diabetes+PhysActiveDays+SmokeNow+AlcoholYear,data=comp_cases)
summary(model_3)
```
Again, BMI is still significant. Other significant confounders:Age,Gender, Race1,Education,DirectChol

##Interaction terms: literature reviewed and some explorations
```{r}
#interaction terms:Literature reviewed
# Age*gender
comp_cases %>%
     ggplot(aes(x=Age, 
               y=BPSysAve, 
               color=Gender))+
     geom_smooth(method="lm")
# BMI*race
comp_cases %>%
     ggplot(aes(x=BMI, 
               y=BPSysAve, 
               color=Race1))+
     geom_smooth(method="lm")
# Diabetes*cholesterol
comp_cases %>%
     ggplot(aes(x=DirectChol,
               y=BPSysAve,
               color=Diabetes))+
     geom_smooth(method="lm")
#Gender*Race
interaction.plot(x.factor = comp_cases$Gender, 
                 trace.factor = comp_cases$Race1,
                 response = comp_cases$BPSysAve)
#Race*education
interaction.plot(x.factor = comp_cases$Race1, 
                 trace.factor = comp_cases$Education,
                 response = comp_cases$BPSysAve)
```
#I was curious if we added well known interactions not involving race or BMI how would the model change. I also looked at how the model changes when the BMI*Race1 is added 
```{r}
#Is there an association when accounting for interactions
model_3a<-lm(BPSysAve~BMI+
                  Age+Gender+Race1+Education+
                  DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+
                  Age*Gender+DirectChol*Diabetes,
                data=comp_cases)
summary(model_3a)

#model with BMI*Race interaction of interest and two well known SBP interactions
#BMI Main effect is no longer significant
model_4<-lm(BPSysAve~BMI+
                  Age+Gender+Race1+Education+
                  DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+
                  Age*Gender+BMI*Race1+DirectChol*Diabetes,
                data=comp_cases)
summary(model_4)

plot_model(model_4,type="diag",show.data = TRUE)
```

#We found other significant interactions with race in the literature, so lets also add them in here. This is the final model. 
```{r}
model_final<-lm(BPSysAve~BMI+
                  Age+Gender+Race1+Education+
                  DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+
                  Age*Gender+BMI*Race1+DirectChol*Diabetes+Gender*Race1+Race1*Education,
                data=comp_cases)
summary(model_final)

plot_model(model_final,type="diag",show.data = TRUE)
```

#Interactions: Exploratory between significant main effects only(Age,Gender, Race1,Education,DirectChol)
```{r}
#Race*cholesterol-exploratory 
comp_cases %>%
     ggplot(aes(x=DirectChol,
               y=BPSysAve,
               color=Race1))+
     geom_smooth(method="lm")
#Race*Age-exploratory 
comp_cases %>%
     ggplot(aes(x=Age,
               y=BPSysAve,
               color=Race1))+
     geom_smooth(method="lm")
#DirectChol*Age-exploratory
quantile(comp_cases$Age)
comp_cases$Age_quan<-ifelse(comp_cases$Age<34, 1,
                            ifelse(34<=comp_cases$Age & comp_cases$Age<47,2,
                                   ifelse(47<=comp_cases$Age & comp_cases$Age<60, 3,
                                   ifelse(comp_cases$Age>=60,4,NA))))
comp_cases %>%
     ggplot(aes(x=DirectChol,
               y=BPSysAve,
               color=as.factor(Age_quan)))+
     geom_smooth(method="lm")
#DirectChol*Gender-exploratory
comp_cases %>%
     ggplot(aes(x=DirectChol,
               y=BPSysAve,
               color=Gender))+
     geom_smooth(method="lm")
#DirectChol*Education-exploratory
comp_cases %>%
     ggplot(aes(x=DirectChol,
               y=BPSysAve,
               color=Education))+
     geom_smooth(method="lm")
#Age*Education-exploratory
comp_cases %>%
     ggplot(aes(x=Age,
               y=BPSysAve,
               color=Education))+
     geom_smooth(method="lm")
#Gender*education-exploratory
interaction.plot(x.factor = comp_cases$Gender, 
                 trace.factor = comp_cases$Education,
                 response = comp_cases$BPSysAve)
```

#From exploration above, I've added these terms to see how it effects our model's adjusted R^2 
```{r}
#from exploration I've added these terms to see how it effects our model's R^2 
#Interactions: Gender*education,Age*DirectChol,Race1*DirectChol
model_5<-lm(BPSysAve~BMI+
                  Age+Gender+Race1+Education+
                  DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+
                  Age*Gender+BMI*Race1+DirectChol*Diabetes+Gender*Race1+Race1*Education+
                  Age*DirectChol+Gender*Education+Race1*DirectChol,
                data=comp_cases)
summary(model_5)

```
Adjusted R^2 goes from 0.237 to 0.241. There is some additional specificity but not a substantial amount

#My attempt at some diagnostics
```{r}
#install.packages("Hmisc")
m.h = hatvalues(model_final)
Hmisc::describe(m.h)

max(m.h)
m.h[which(m.h>0.5)]#high leverage-0
m.h[which(m.h>0.2)]#moderate leverage-14
```
```{r}
#install.packages("olsrr")
options(width = 70)
options(warn=-1)

m.dffits=dffits(model_4)
m.dfbeta=dfbeta(model_4)
m.D=cooks.distance(model_4)
m.covratio=covratio(model_4)

n = nrow(comp_cases); p = model_4$rank
plot(model_4, which=4); abline(h=4/n,lty=2) #cooks difference: impact on the parameter estimates
plot(abs(m.covratio-1)); abline(1+3*p/n,0); abline(1-3*p/n,0) #influence on the precision of estimation of beta_hat
olsrr::ols_plot_dfbetas(model_4)
olsrr::ols_plot_dffits(model_4)
```
```{r}
#Compared the complete cases to the target population
model_pop<-lm(BPSysAve~BMI+
                  Age+Gender+Race1+Education+
                  DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+
                  Age*Gender+BMI*Race1+DirectChol*Diabetes+Gender*Race1+Race1*Education,data=target_pop)
summary(model_pop)
```

#partial regression plots
```{r}
car::avPlots(model_final)
car::residualPlots(model_final,type="response")
```

